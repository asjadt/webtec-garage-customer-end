...
const DashboardWidgetWithContent = ({
  ...
  const graphData =
    item?.data !== null || item?.data !== undefined
      ? Object.keys(item?.data || {})?.length > 0
        ? {
            labels: Object.keys(item?.data)?.map((d) => formatRole(d)),
  ...
  return (
    ...
    >
      <div className={`flex justify-start items-center`}>
        <div className={`flex items-start gap-2 md:gap-3  w-full`}>
          ...
          >
            ...
            {/* TOTAL COUNTER  */}
            {/* <span
              className={`w-8 h-8 rounded-lg bg-primary text-base-300 flex justify-center items-center `}
            >
              {item?.total_data_count || 0}
            </span> */}
          </div>
        </div>
      </div>
      ...
      {/*
      <div
        className={`bg-primary w-20 h-20 absolute blur-2xl -right-10  -bottom-10`}
      ></div>
      <div
        className={`bg-cyan-500 w-6 h-6 absolute blur-xl right-10  -bottom-5`}
      ></div> */}
    </div>
  );
};
