...
export default function TextAreaFieldV2({
  control,
  id,
  name,
  label,
  ...
  return (
    <div
      data-cy={`view-employee-container-textarea-field-v2`}
      className={`flex flex-col gap-y-2 ${wrapperClassName}`}
    >
      {/* LABEL */}
      ...
      {/* INPUT FIELD */}
      <Controller
        data-cy={`view-employee-textarea-textarea-field-v2`}
        ...
        render={({ field, fieldState: { invalid, error } }) => (
          <textarea
            ...
            className={`input bg-base-300 focus:outline-primary scrollbar-thin scrollbar-thumb-primary-content input-bordered ${height} pt-3
              ${fieldClassName}
              ${
                disabled &&
                `px-1 py-0 border
      ...
      <ErrorMessage
        ...
        render={({ message }) => (
          <label
            data-cy={`view-employee-error-message-textarea-field-v2`}
            className="label h-7"
          >
            <p
