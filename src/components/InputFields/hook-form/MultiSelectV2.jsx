...
const MultiSelectV2 = forwardRef(
  (
    ...
    // const selectRef = useClickOutside(() => setIsDropdownOpen(false));
    ...
    useEffect(() => {
      ...
      //   IF ONLY 1 OPTION  THEN IT WILL BY DEFAULT SET
      // if (optionsMemo.length === 1) {
      // setSelectedOptions(optionsMemo);
    ...
    return (
      <div
        className={`relative w-full min-w-96 ${disable ? "cursor-not-allowed opacity-50" : ""
          }
          ${wrapperClassName}
          `}
        onClick={(e) => {
      ...
      >
        ...
        >
          {selectedOptions.map((option) => (
            ...
            >
              {typeof option?.label === "string" && truncateText(option?.label, 20)}{" "}
              {!disable && (
                <button
                  onClick={(e) => {
                    // e.preventDefault();
          ...
          {isSearchable && (
            <input
              ...
              className={`w-full bg-base-300  h-11 flex-grow p-1 outline-none  ${(singleSelect && selectedOptions.length) || disable
                ? "hidden"
                : ""
                }`}
            />
        ...
        {isDropdownOpen && !disable && (
          <div
            ref={dropdownRef}
            className={`absolute ${top ? "bottom-full -mb-7" : "top-full mt-2"
              } z-30 bg-base-300 duration-200 transition-all overflow-hidden  ${isDropdownOpen
          ...
          >
            ...
            {filteredOptions.length > 0 ? (
              filteredOptions.map((option) => (
                <label
                  key={option.id}
                  className={`px-5 py-1   justify-between w-full flex items-center hover:cursor-pointer text-base-content border-b border-solid border-gray-300  ${selectedOptions.some((item) => item.id === option.id)
