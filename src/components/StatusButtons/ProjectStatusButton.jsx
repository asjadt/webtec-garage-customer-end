...
const ProjectStatusButton = ({ d }) => {
  return (
    <div className={`md:flex justify-center`}>
      ...
      >
        <div
          className={`w-4 h-4 border-2  ${d?.status === "pending" && "border-gray-500"
            } ${d?.status === "approved" && "  border-green-500"}  ${d?.status === "in_progress" && "border-blue-500"
            } rounded-full flex justify-center items-center mr-1`}
        >
          <div
            className={`w-2 h-2 border-2 ${d?.status === "pending" && "border-gray-500 bg-gray-500"
